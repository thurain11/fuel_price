...
class ExternalRefreshUiBuilder<T, D> extends StatefulWidget {
  ...
  @override
  ExternalRefreshUiBuilderState createState() =>
      ExternalRefreshUiBuilderState<T, D>(this.map);
}

class ExternalRefreshUiBuilderState<T, D>
    extends State<ExternalRefreshUiBuilder> {
      ...
      builder: (context, AsyncSnapshot<ResponseOb> snap) {
        ...
        if (rv.message == MsgState.loading) {
              ...
              footer: CustomFooter(
                builder: (context, loadStatus) {
                  if (loadStatus == LoadStatus.loading) {
                    ...
                    return Center(
                      child: Text('Pull up to load', style: pullUpSty),
                    );
                  } else {
                    return Container();
                  }
          ...
          if (rv.errState == ErrState.no_internet) {
            ...
            return ConnectionTimeoutWidget(
              fun: () {
                bloc.getData(
                  widget.url,
                  map: widget.map,
  ...
  Widget mainList(List<D>? ois) {
    return widget.isList
        ? ListView.builder(
            ...
            physics: widget.scorllableHeaderWidget == null
                ? null
